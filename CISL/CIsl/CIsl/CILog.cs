...
namespace CIsl
{
    ...
    public class CILog
    {
        ...
        /// 実体
        /// </summary>
        private static CILog Instance = null;

        /// <summary>
        /// 取得プロパティ
        /// </summary>
        public static CILog Log
        {
            get
            {
                if (CILog.Instance == null)
                {
                    CILog.Instance = new CILog();
                }

                return CILog.Instance;
            }
        }
        ...
        /// ログファイル名
        /// </summary>
        private string LogFileName = "system.log";
        ...
        public static bool WriteLog(string s, bool f = true)
        {
            lock (CILog.Instance)
            {
                return CILog.Log.WriteLogData(s, f);
            }
        }
        ...
        public static bool WriteLog(Exception e, string s = "")
        {
            lock (CILog.Instance)
            {
                CILog.Log.WriteLogData("------" + s + "Exception------", true);
                CILog.Log.WriteLogData("Message = " + e.Message, false);
                CILog.Log.WriteLogData("Source = " + e.Source, false);
                CILog.Log.WriteLogData("StackTrace = " + e.StackTrace, false);
                CILog.Log.WriteLogData("TargetSite = " + e.TargetSite.Name, false);

                Exception epc = e.InnerException;
                while (epc != null)
                {
                    CILog.Log.WriteLogData("Innser Message = " + epc.Message, false);
                    epc = epc.InnerException;
                }


                CILog.Log.WriteLogData("------------------", false);
        ...
        public static bool WriteLog(object src, string s, bool f = true)
        {
            lock (CILog.Instance)
            {
                ...
                return CILog.Log.WriteLogData(mes, f);
            }
        }
