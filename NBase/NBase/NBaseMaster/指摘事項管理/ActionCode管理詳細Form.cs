...
{
    public partial class ActionCode管理詳細Form : Form
    {
        private MsActionCode actionCode;
		
		//データを変更したかどうか？
		private bool ChangeFlag = false;


        public ActionCode管理詳細Form() : this(new MsActionCode())
        {
        }


        public ActionCode管理詳細Form(MsActionCode actionCode)
        {
        ...
        private void Init()
        {
            if (!actionCode.IsNew())
            {
                textBox名.Text = actionCode.ActionCodeName;
                textBoxテキスト.Text = actionCode.ActionCodeText;
            }
            else
            {
        ...
        private bool ValidateFields()
        {
            if (textBox名.Text.Length == 0)
            {
                ...
                MessageBox.Show("ActionCode名を入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox名.BackColor = Color.White;
                return false;
            }
            else if (textBoxテキスト.Text.Length == 0)
            {
                textBoxテキスト.BackColor = Color.Pink;
                MessageBox.Show("Actionテキストを入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBoxテキスト.BackColor = Color.White;
                return false;
            }
        ...
        {
            actionCode.ActionCodeName = textBox名.Text;
            actionCode.ActionCodeText = textBoxテキスト.Text;
        }
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                result = serviceClient.MsActionCode_InsertOrUpdate(NBaseCommon.Common.LoginUser, actionCode);
            }
        ...
        private void button削除_Click(object sender, EventArgs e)
        {
            ...
            //削除前に使用しているかのチェックをする
            bool ret = this.CheckDeleteUsing(this.actionCode);
        ...
        /// <summary>
        /// 対象のMsActionCodeデータが使用されているかを調べる
        /// 引数：チェックするデータ
        /// 返り値：削除可能→true、使用されている→false
        /// </summary>
        /// <returns></returns>
        private bool CheckDeleteUsing(MsActionCode data)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
            }
