...
{
    public partial class 発生状況管理詳細Form : Form
    {
        private MsAccidentSituation accidentSituation;
		
		//データを変更したかどうか？
		private bool ChangeFlag = false;


        public 発生状況管理詳細Form() : this(new MsAccidentSituation())
        {
        }


        public 発生状況管理詳細Form(MsAccidentSituation accidentSituation)
        {
        ...
        private void Init()
        {
            if (!accidentSituation.IsNew())
            {
                textBox名.Text = accidentSituation.AccidentSituationName;
            }
            else
            {
        ...
        private bool ValidateFields()
        {
            if (textBox名.Text.Length == 0)
            {
                ...
                MessageBox.Show("発生状況名を入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                textBox名.BackColor = Color.Pink;
                MessageBox.Show("発生状況名は20文字以下で入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox名.BackColor = Color.White;
                return false;
            }
        ...
        {
            accidentSituation.AccidentSituationName = textBox名.Text;
        }
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                result = serviceClient.MsAccidentSituation_InsertOrUpdate(NBaseCommon.Common.LoginUser, accidentSituation);
            }
        ...
        private void button削除_Click(object sender, EventArgs e)
        {
            ...
            //削除前に使用しているかのチェックをする
            bool ret = this.CheckDeleteUsing(this.accidentSituation);
            ...
            {
                accidentSituation.DeleteFlag = true;
                Save(true);
            }
        }
        ...
        /// <summary>
        /// 対象のMsAccidentSituationデータが使用されているかを調べる
        /// 引数：チェックするデータ
        /// 返り値：削除可能→true、使用されている→false
        /// </summary>
        /// <returns></returns>
        private bool CheckDeleteUsing(MsAccidentSituation data)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
            }
