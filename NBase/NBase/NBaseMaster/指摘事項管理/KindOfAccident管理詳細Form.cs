...
{
    public partial class KindOfAccident管理詳細Form : Form
    {
        private MsKindOfAccident kindOfAccident;
		
		//データを変更したかどうか？
		private bool ChangeFlag = false;


        public KindOfAccident管理詳細Form() : this(new MsKindOfAccident())
        {
        }


        public KindOfAccident管理詳細Form(MsKindOfAccident kindOfAccident)
        {
        ...
        private void Init()
        {
            if (!kindOfAccident.IsNew())
            {
                textBox名.Text = kindOfAccident.KindOfAccidentName;
            }
            else
            {
        ...
        private bool ValidateFields()
        {
            if (textBox名.Text.Length == 0)
            {
                ...
                MessageBox.Show("KindOfAccident名を入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                textBox名.BackColor = Color.Pink;
                MessageBox.Show("KindOfAccident名は20文字以下で入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox名.BackColor = Color.White;
                return false;
            }
        ...
        {
            kindOfAccident.KindOfAccidentName = textBox名.Text;
        }
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                result = serviceClient.MsKindOfAccident_InsertOrUpdate(NBaseCommon.Common.LoginUser, kindOfAccident);
            }
        ...
        private void button削除_Click(object sender, EventArgs e)
        {
            ...
            //削除前に使用しているかのチェックをする
            bool ret = this.CheckDeleteUsing(this.kindOfAccident);
            ...
            {
                kindOfAccident.DeleteFlag = true;
                Save(true);
            }
        }
        ...
        /// <summary>
        /// 対象のMsKindOfAccidentデータが使用されているかを調べる
        /// 引数：チェックするデータ
        /// 返り値：削除可能→true、使用されている→false
        /// </summary>
        /// <returns></returns>
        private bool CheckDeleteUsing(MsKindOfAccident data)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
            }
