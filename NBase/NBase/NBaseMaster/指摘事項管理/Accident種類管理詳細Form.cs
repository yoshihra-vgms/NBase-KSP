...
{
    public partial class Accident種類管理詳細Form : Form
    {
        private MsAccidentKind accidentKind;
		
		//データを変更したかどうか？
		private bool ChangeFlag = false;


        public Accident種類管理詳細Form() : this(new MsAccidentKind())
        {
        }


        public Accident種類管理詳細Form(MsAccidentKind accidentKind)
        {
        ...
        private void Init()
        {
            if (!accidentKind.IsNew())
            {
                textBox名.Text = accidentKind.AccidentKindName;
            }
            else
            {
        ...
        private bool ValidateFields()
        {
            if (textBox名.Text.Length == 0)
            {
                ...
                MessageBox.Show("Accident種類名を入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                textBox名.BackColor = Color.Pink;
                MessageBox.Show("Accident種類名は20文字以下で入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox名.BackColor = Color.White;
                return false;
            }
        ...
        {
            accidentKind.AccidentKindName = textBox名.Text;
        }
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                result = serviceClient.MsAccidentKind_InsertOrUpdate(NBaseCommon.Common.LoginUser, accidentKind);
            }
        ...
        private void button削除_Click(object sender, EventArgs e)
        {
            ...
            //削除前に使用しているかのチェックをする
            bool ret = this.CheckDeleteUsing(this.accidentKind);
        ...
        /// <summary>
        /// 対象のMsAccidentKindデータが使用されているかを調べる
        /// 引数：チェックするデータ
        /// 返り値：削除可能→true、使用されている→false
        /// </summary>
        /// <returns></returns>
        private bool CheckDeleteUsing(MsAccidentKind data)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
            }
