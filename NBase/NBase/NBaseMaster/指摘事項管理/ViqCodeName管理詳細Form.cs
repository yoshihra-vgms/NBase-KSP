...
{
    public partial class ViqCodeName管理詳細Form : Form
    {
        private MsViqCodeName viqCodeName;
		
		//データを変更したかどうか？
		private bool ChangeFlag = false;


        public ViqCodeName管理詳細Form() : this(new MsViqCodeName())
        {
        }


        public ViqCodeName管理詳細Form(MsViqCodeName viqCodeName)
        {
            this.viqCodeName = viqCodeName;
        ...
        private void Init()
        {
            if (!viqCodeName.IsNew())
            {
                textBox名.Text = viqCodeName.ViqCodeName;
                textBoxCode.Text = viqCodeName.Description;
                textBoxCodeEng.Text = viqCodeName.DescriptionEng;
                textBoxOrderNo.Text = viqCodeName.OrderNo.ToString();
            }
            else
            {
        ...
        {
            viqCodeName.ViqCodeName = textBox名.Text;
            viqCodeName.Description = textBoxCode.Text;
            viqCodeName.DescriptionEng = textBoxCodeEng.Text;
            viqCodeName.OrderNo = int.Parse(textBoxOrderNo.Text);
        }
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                result = serviceClient.MsViqCodeName_InsertOrUpdate(NBaseCommon.Common.LoginUser, viqCodeName);
            }
        ...
        private void button削除_Click(object sender, EventArgs e)
        {
            ...
            //削除前に使用しているかのチェックをする
            bool ret = this.CheckDeleteUsing(this.viqCodeName);
        ...
        /// <summary>
        /// 対象のMsViqCodeNameデータが使用されているかを調べる
        /// 引数：チェックするデータ
        /// 返り値：削除可能→true、使用されている→false
        /// </summary>
        /// <returns></returns>
        private bool CheckDeleteUsing(MsViqCodeName data)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
            }
