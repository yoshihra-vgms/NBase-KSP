...
{
    public partial class ViqNo管理詳細Form : Form
    {
        private MsViqNo viqNo;
        private List<MsViqCode> codes;
	
		//データを変更したかどうか？
		private bool ChangeFlag = false;


        public ViqNo管理詳細Form(List<MsViqCode> codes)
            : this(codes, new MsViqNo())
        {
        }


        public ViqNo管理詳細Form(List<MsViqCode> codes, MsViqNo viqNo)
        {
            this.codes = codes;
            this.viqNo = viqNo;
        ...
        private void Init()
        {
            ...
            if (!viqNo.IsNew())
            {
                var c = codes.Where(obj => obj.ViqCodeID == viqNo.ViqCodeID).First();
                comboBoxCode.SelectedItem = c;

                textBox名.Text = viqNo.ViqNo;
                textBoxCode.Text = viqNo.Description;
                textBoxCodeEng.Text = viqNo.DescriptionEng;
                textBoxOrderNo.Text = viqNo.OrderNo.ToString();
            }
            else
            {
        ...
        private bool ValidateFields()
        {
            ...
            {
                comboBoxCode.BackColor = Color.Pink;
                MessageBox.Show("VIQ Codeを選択して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            else if (textBox名.Text.Length == 0)
            {
                ...
                MessageBox.Show("VIQ Noを入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        {
            viqNo.ViqCodeID = (comboBoxCode.SelectedItem as MsViqCode).ViqCodeID;
            viqNo.ViqNo = textBox名.Text;
            viqNo.Description = textBoxCode.Text;
            viqNo.DescriptionEng = textBoxCodeEng.Text;
            viqNo.OrderNo = int.Parse(textBoxOrderNo.Text);
        }
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                result = serviceClient.MsViqNo_InsertOrUpdate(NBaseCommon.Common.LoginUser, viqNo);
            }
        ...
        private void button削除_Click(object sender, EventArgs e)
        {
            ...
            //削除前に使用しているかのチェックをする
            bool ret = this.CheckDeleteUsing(this.viqNo);
        ...
        /// <summary>
        /// 対象のMsViqNoデータが使用されているかを調べる
        /// 引数：チェックするデータ
        /// 返り値：削除可能→true、使用されている→false
        /// </summary>
        /// <returns></returns>
        private bool CheckDeleteUsing(MsViqNo data)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
            }
