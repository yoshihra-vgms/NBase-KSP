...
{
    public partial class 検船種別管理詳細Form : Form
    {
        private MsInspectionCategory inspectionCategory;
		
		//データを変更したかどうか？
		private bool ChangeFlag = false;


        public 検船種別管理詳細Form() : this(new MsInspectionCategory())
        {
        }


        public 検船種別管理詳細Form(MsInspectionCategory inspectionCategory)
        {
        ...
        private void Init()
        {
            if (!inspectionCategory.IsNew())
            {
                textBox名.Text = inspectionCategory.InspectionCategoryName;
            }
            else
            {
        ...
        private bool ValidateFields()
        {
            if (textBox名.Text.Length == 0)
            {
                ...
                MessageBox.Show("検船種別名を入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
            ...
            {
                textBox名.BackColor = Color.Pink;
                MessageBox.Show("検船種別名は20文字以下で入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox名.BackColor = Color.White;
                return false;
            }
        ...
        {
            inspectionCategory.InspectionCategoryName = textBox名.Text;
        }
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                result = serviceClient.MsInspectionCategory_InsertOrUpdate(NBaseCommon.Common.LoginUser, inspectionCategory);
            }
        ...
        private void button削除_Click(object sender, EventArgs e)
        {
            ...
            //削除前に使用しているかのチェックをする
            bool ret = this.CheckDeleteUsing(this.inspectionCategory);
            ...
            {
                inspectionCategory.DeleteFlag = true;
                Save(true);
            }
        }
        ...
        /// <summary>
        /// 対象のMsInspectionCategoryデータが使用されているかを調べる
        /// 引数：チェックするデータ
        /// 返り値：削除可能→true、使用されている→false
        /// </summary>
        /// <returns></returns>
        private bool CheckDeleteUsing(MsInspectionCategory data)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
            }
