...
namespace Yojitsu.util
{
	public partial class 経常収支管理表出力処理
	{
		...
		///// <param name="rate"></param>
		///// <param name="head"></param>
		//private void WriteFileMain(int sy, int sm, int ey, int em,
		//    string unit, decimal rate, BgYosanHead head)
		//{
		//    //タイトルの書き込み
		//    this.Write題名();
		...
		///// タイトルの書き込み
		///// </summary>
		//private void Write題名()
		//{
		//    //タイトルのフォント変更
		//    this.Crea.Cell(util.経常収支管理表出力処理.Pos題名).Attr.FontPoint = util.経常収支管理表出力処理.TitleRow文字サイズ;
		//    //this.Crea.Cell(util.経常収支管理表出力処理.Pos題名).Attr.FontStyle = xlFontStyle.xsBold;

		//    //名前を書き込み
		//    this.Crea.Cell(util.経常収支管理表出力処理.Pos題名).Value = util.経常収支管理表出力処理.TitleName;
		//}

		///// <summary>
		///// Colで何を描くかを決定する(船タイプ以外)
		///// 引数：関連Colデータ
		///// </summary>
		///// <param name="col"></param>
		//private void SelectWriteData(ColInfo col)
		//{
		//    //船のとき
		//    if (col.Type == ColInfo.TYPE.VESSEL)
		//    {
		//        //なにもなし
		//        return;
		//    }
		//    //////////////////////////////////////////////////////////
		//    //内航船合計のとき
		//    if (col.Type == ColInfo.TYPE.VES_SUM)
		//    {
		//        this.WriteGrapth内航船合計();
		//        return;
		//    }
		//    //////////////////////////////////////////////////////////
		//    //外航船のとき
		//    if (col.Type == ColInfo.TYPE.OUT_VES)
		//    {
		//        this.WriteGrapth外航船();
		//        return;
		//    }
		//    //////////////////////////////////////////////////////////
		//    //全体合計のとき
		//    if (col.Type == ColInfo.TYPE.ALL_SUM)
		//    {
		//        this.WriteGrapth全体合計();
		//        return;
		//    }
		//}
		...
		//private void WriteGrapth内航船合計()
		//{
		//    string sc;
		//    string ec;

		//    //計算Cellの取得
		//    this.SearchMinMaxCol(out sc, out ec);
		...
		//        //書き込み
		//        string spos = this.VesselSum.Col + r.ToString();
		//        this.Crea.Cell(spos).Value = sfunc;

		//        //線の変更
		//        this.Crea.Cell(spos).Attr.LineBottom(ExcelCreator.BorderStyle.Dashed, ExcelCreator.xlColor.Black);
		//        this.Crea.Cell(spos).Attr.LineRight(ExcelCreator.BorderStyle.Thin, ExcelCreator.xlColor.Black);

		//        //実績は太字
		//        this.Crea.Cell(spos).Attr.FontStyle = xlFontStyle.xsBold;
		...
		//        //書き込み
		//        spos = this.VesselSum.Col + r.ToString();
		//        this.Crea.Cell(spos).Value = sfunc;


		//        //線の変更
		//        this.Crea.Cell(spos).Attr.LineTop(ExcelCreator.BorderStyle.Dashed, ExcelCreator.xlColor.Black);
		//        this.Crea.Cell(spos).Attr.LineRight(ExcelCreator.BorderStyle.Thin, ExcelCreator.xlColor.Black);
		...
		//        ////////////////////////////////////////////////////////////
		//        //実績
		//        string spos = this.OutVessel.Col + r.ToString();
		//        this.Crea.Cell(spos).Value = 0;

		//        //線の変更
		//        this.Crea.Cell(spos).Attr.LineBottom(ExcelCreator.BorderStyle.Dashed, ExcelCreator.xlColor.Black);
		//        this.Crea.Cell(spos).Attr.LineLeft(ExcelCreator.BorderStyle.Thin, ExcelCreator.xlColor.Black);
		//        this.Crea.Cell(spos).Attr.LineRight(ExcelCreator.BorderStyle.Thin, ExcelCreator.xlColor.Black);
		...
		//        ////////////////////////////////////////////////////////////
		//        //計画合計
		//        spos = this.OutVessel.Col + r.ToString();
		//        this.Crea.Cell(spos).Value = 0;
		...
		//private void WriteGrapth全体合計()
		//{
		//    //内航船合計と外航船を足し合わせる
		//    string inc = this.VesselSum.Col;
		//    string outc = this.OutVessel.Col;
		...
		//        //実績
		//        sfunc = "=";
		//        sfunc += inc + r.ToString();
		//        sfunc += "+";
		//        sfunc += outc + r.ToString();


		//        string spos = this.AllSum.Col + r.ToString();
		//        this.Crea.Cell(spos).Value = sfunc;
		...
		//        spos = this.AllSum.Col + r.ToString();
		//        this.Crea.Cell(spos).Value = sfunc;
		...
		///// <summary>
		///// 船種別の書き込み場所の最高と最低を取得
		///// 本当は全てをしっかり調べたほうがいいんだろうけどはしょって初めと最後をとってくるようにしてある
		///// </summary>
		///// <param name="s"></param>
		///// <param name="e"></param>
		//private void SearchMinMaxCol(out string sout, out string eout)
		//{
		//    sout = this.ColInfoList[0].Col;
		//    eout = "A";

		//    foreach (ColInfo c in this.ColInfoList)
		//    {

		//        if (c.VesselTypeData == null)
		//        {
		//            return;
		//        }
		...
		private void CreateColInfo()
		{
			...
			vtlist = DbAccessorFactory.FACTORY.MsVesselType_GetRecords(NBaseCommon.Common.LoginUser);
		...
		private void CreateRowInfo()
		{
			...
			himolist = DbAccessorFactory.FACTORY.MsHimoku_GetRecords(NBaseCommon.Common.LoginUser);


			//今の位置
			int pos = Convert.ToInt32(util.経常収支管理表出力処理.Grapth開始位置[1].ToString());
			pos += 1;
		...
		{
			//シートNo設定
			this.Crea.SheetNo = util.経常収支管理表出力処理.費目テンプレートSheetNo;
		...
		private void Write年月(int sy, int sm, int ey, int em)
		{
			...
			//年を書き込み
			this.Crea.Cell(util.経常収支管理表出力処理.Temp年月).Value = sdate;
		}
		...
		private void Write単位(string unit)
		{
			...
			//書き込み
			this.Crea.Cell(util.経常収支管理表出力処理.Temp単位).Value = sdata;
		}
		...
		{
			//船を取得
			List<MsVessel> vessellist = DbAccessorFactory.FACTORY.MsVessel_GetRecords(NBaseCommon.Common.LoginUser);
			//List<MsVessel> vessellist = DbAccessorFactory.FACTORY.MsVessel_GetRecordsAll(NBaseCommon.Common.LoginUser);
		...
		private string TemplateFileName作成()
		{
			...
			string name = path + "/config/" + util.経常収支管理表出力処理.経常収支Templateファイル名;
		...
		private void WriteGraphHeader()
		{
			...
			int colpos = Convert.ToInt32(util.経常収支管理表出力処理.Grapth開始位置[1].ToString());
			...
			string rowpos = util.経常収支管理表出力処理.Grapth開始位置[0].ToString();
		...
		{
			string spos = util.経常収支管理表出力処理.Grapth開始位置;
			...
			string unit, decimal rate, BgYosanHead head)
		{
			...
			foreach (ColInfo colinfo in this.ColInfoList)
			{
				...
				//	DbAccessorFactory.FACTORY.BgYosanItem_GetRecordsByYosanHeadPriodVesselTypeBumonIDHimokus
				//	(NBaseCommon.Common.LoginUser, head.YosanHeadID,
				//	start_date, end_date, colinfo.VesselTypeData.MsVesselTypeID,
				//	util.経常収支管理表出力処理.営業GBumonID);
				List<BgYosanItem> planlist =
						DbAccessorFactory.FACTORY.BgYosanItem_GetRecordsByYosanHeadPriodVesselTypeHimokus
						(NBaseCommon.Common.LoginUser, head.YosanHeadID,
						start_date, end_date, colinfo.VesselTypeData.MsVesselTypeID);
				...
				List<BgJiseki> exlist =
					DbAccessorFactory.FACTORY.BgJiseki_GetRecordsByVesselTypePriodHimokus
					(NBaseCommon.Common.LoginUser, colinfo.VesselTypeData.MsVesselTypeID,
					start_date, end_date);
		...
		{
			//書き込み場所の作成
			string cellpos = col + (rowinfo.Row + util.経常収支管理表出力処理.計画ColOffset).ToString();
			...
			if (data < 0)
			{
				this.Crea.Cell(cellpos).Attr.FontColor = util.経常収支管理表出力処理.MinusFontColor;
			}
		...
		{
			//書き込み場所の作成
			string cellpos = col + (rowinfo.Row + util.経常収支管理表出力処理.実績ColOffset).ToString();
