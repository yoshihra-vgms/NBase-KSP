    ...
    internal class TreeListViewDelegate船内収支 : TreeListViewDelegate
    {
        ...
        internal void SetRows(DateTime month, DateTime 先月締め日, decimal 先月末残高, List<SiJunbikin> junbikins)
        {
            ...
            for (int i = 0; i < junbikins.Count; i++)
            {
                ...
                AddSubItem(node, SeninTableCache.instance().GetMsSiHiyouKamokuName(NBaseCommon.Common.LoginUser, j.MsSiHiyouKamokuID), true);
                AddSubItem(node, SeninTableCache.instance().GetMsSiDaikoumokuName(NBaseCommon.Common.LoginUser, j.MsSiDaikoumokuID), true);
                ...
                AddSubItem(node, SeninTableCache.instance().GetMsSiKamokuName(NBaseCommon.Common.LoginUser, j.MsSiKamokuId), true);
                AddSubItem(node, j.FurikaeFlag == 1 ? "○" : string.Empty, true);
