...
namespace Senin
{
    public partial class 賞罰詳細Form : Form
    {
        private 船員詳細Panel parentForm;
        private SiShobatsu shobatsu;
        private bool isNew;

        public 賞罰詳細Form(船員詳細Panel parentForm)
            : this(parentForm, new SiShobatsu(), true)
        {
        }


        public 賞罰詳細Form(船員詳細Panel parentForm, SiShobatsu shobatsu, bool isNew)
        {
            this.parentForm = parentForm;
            this.shobatsu = shobatsu;
            this.isNew = isNew;
        ...
        private void InitFields()
        {
            if (!isNew)
            {
                dateTimePicker年月日.Value = shobatsu.ShobatsuDate;
                textBox賞罰名.Text = shobatsu.ShobatsuName;
                textBox備考.Text = shobatsu.Remarks;

                foreach (SiShobatsuAttachFile attach in shobatsu.AttachFiles)
                {
        ...
        private void Save()
        {
            if (ValidateFields())
            {
                ...
                if (parentForm.InsertOrUpdate_賞罰詳細(shobatsu))
                {
        ...
        {
            if (textBox賞罰名.Text.Length == 0)
            {
                textBox賞罰名.BackColor = Color.Pink;
                MessageBox.Show("賞罰名を入力して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox賞罰名.BackColor = Color.White;
                return false;
            }
        ...
        {
            shobatsu.ShobatsuDate = dateTimePicker年月日.Value;
            shobatsu.ShobatsuName = StringUtils.Escape(textBox賞罰名.Text);
            shobatsu.Remarks = StringUtils.Escape(textBox備考.Text);       
        }
        ...
        private void button添付削除_Click(object sender, EventArgs e)
        {
            ...
            if (listBox添付ファイル.SelectedItem is SiShobatsuAttachFile)
            {
                if (MessageBox.Show("選択したファイルを削除しますか？", "", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    SiShobatsuAttachFile deleteAttach = listBox添付ファイル.SelectedItem as SiShobatsuAttachFile;

                    for (int i = 0; i < shobatsu.AttachFiles.Count; i++)
                    {
                        SiShobatsuAttachFile attach = shobatsu.AttachFiles[i];
                        if (attach.SiShobatsuAttachFileID == deleteAttach.SiShobatsuAttachFileID)
                        {
                            if (attach.SiShobatsuID == null)
                            {
                                shobatsu.AttachFiles.Remove(attach);
                            }
                            else
                            {
                                attach.DeleteFlag = 1;
                                shobatsu.AttachFiles[i] = attach;
                            }
                            break;
                        }
                    }
        ...
        private void listBox添付ファイル_DoubleClick(object sender, EventArgs e)
        {
            if (listBox添付ファイル.SelectedItem is SiShobatsuAttachFile)
            {
                SiShobatsuAttachFile attach = (listBox添付ファイル.SelectedItem as SiShobatsuAttachFile);
                // ファイルの表示
                NBaseCommon.FileView.View(attach.SiShobatsuAttachFileID, attach.FileName, attach.Data);
            }
        }
        ...
        {
            SiShobatsuAttachFile attach = new SiShobatsuAttachFile();
            attach.SiShobatsuAttachFileID = System.Guid.NewGuid().ToString();
            ...
            shobatsu.AttachFiles.Add(attach);
            listBox添付ファイル.Items.Add(attach);
