...
namespace Senin
{
    public partial class 配乗計画内航Form : Form
    {
        private static 配乗計画内航Form instance;
        ...
        /// </summary>
        /// <returns></returns>
        public static 配乗計画内航Form Instance()
        {
            if (instance == null)
            {
                instance = new 配乗計画内航Form();
            }
        ...
        /// <param name="e"></param>
        #region private void 配乗計画内航Form_FormClosed(object sender, FormClosedEventArgs e)
        private void 配乗計画内航Form_FormClosed(object sender, FormClosedEventArgs e)
        {
            instance = null;
        }
        #endregion
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        #region private void 配乗計画内航Form_Load(object sender, EventArgs e)
        private void 配乗計画内航Form_Load(object sender, EventArgs e)
        {
            // この配乗表画面のタイプ
            PlanType = MsPlanType.PlanTypeHarfPeriod;
            this.Text = "配乗計画（" + MsPlanType.GetRecords()[MsPlanType.PlanTypeHarfPeriod].Name + "）";
        ...
        private void InitCalendarStyle()
        {
            ...
            //// 2行1列目のセルにユーザー定義のセルスタイルを設定します。
            //gcCalendarGrid1.Template.Content.GetCell(1, 0).CellStyleName = "style";
        }
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        #region private void 配乗計画内航Form_Shown(object sender, EventArgs e)
        private void 配乗計画内航Form_Shown(object sender, EventArgs e)
        {
            InitGridView色見本();
        }
        #endregion
        ...
        private void MakeColorList()
        {
            ...
            //ColorList.Add(Color.FromArgb(198,49,177));//赤紫
            ...
            //ColorList_L.Add(Color.FromArgb(251, 230, 249));//赤紫
        ...
        private void InitCombobox種別()
        {
            ...
            //{
            //    if (v.MsVesselTypeID == MsVesselType.MS_VESSEL_TYPE_内航RORO_ID || v.MsVesselTypeID == MsVesselType.MS_VESSEL_TYPE_内航不定期_ID)
            //    {
        ...
        private void SetDataPlan()
        {
            ...
            if (checkBox_計画.Checked == true)
            {
                ...
                foreach (Appointment plan in PlanViewList)
                {
                    ...
                    {
                        //職名セット
                        Shokumei shoku = SeninTableCache.instance().GetShokumei(NBaseCommon.Common.LoginUser, Shokumei.内航, plan.MsSiShokumeiID, plan.MsSiShokumeiShousaiID);
                    ...
                    //Appointmentを置く
                    PutAppointment(plan, index, plan.StartDate, span, false);
                }
            ...
            if (checkBox_実績.Checked == true)
            {
                ...
                foreach (Appointment ac in ActList)
                {
                    ...
                    {
                        string str = "";
                        if (ac.MsSiShubetsuID == SeninTableCache.instance().MsSiShubetsu_乗船ID(NBaseCommon.Common.LoginUser))
                        {
                            str = "乗船";
                        }
                        else
                        ...
                        System.Diagnostics.Debug.WriteLine(str + ":start=" + ac.StartDate.ToShortDateString() + " enddate=" + ac.EndDate.ToShortDateString()+ " span= " + span.ToString());
                    }


                    //Appointmentを置く
                    PutAppointment(ac, index, ac.StartDate, span, false);
                }
            }
            #endregion
        ...
        private void buttonRevUp_Click(object sender, EventArgs e)
        {
            ...
            {
                //リビジョンをあげる
                //ret = serviceClient.BLC_配乗計画_RevisionUp(NBaseCommon.Common.LoginUser, dt, SiCardPlanHead.VESSEL_KIND_内航);
        ...
        private void button月締_Click(object sender, EventArgs e)
        {
            ...
            {
                //締める
                //ret = serviceClient.BLC_配乗計画_Shime(NBaseCommon.Common.LoginUser, dt, SiCardPlanHead.VESSEL_KIND_内航);
        ...
        {
            //配乗計画内航出力Form frm = new 配乗計画内航出力Form(PlanType, Periodyear);
            //frm.ShowDialog();
        }
        #endregion
        ...
        private void gcCalendarGrid1_CellMouseClick(object sender, CalendarCellMouseEventArgs e)
        {
            ...
            //{参考コード
            //    //週数とか範囲
            //    CalendarWeekRule rule = gcCalendarGrid1.DateFormatInfo.CalendarWeekRule;
            //    DayOfWeek week = gcCalendarGrid1.DateFormatInfo.FirstDayOfWeek;

            //    CultureInfo cinfo = gcCalendarGrid1.DateFormatInfo.CultureInfo;
            //    Calendar cal = cinfo.Calendar;

            //    int a = cal.GetWeekOfYear(dt, rule, week);

            //    //1週間選択の場合、以下で求める
            //    int wnum = (int)dt.DayOfWeek;
            ...
            //船員が内航籍かどうか
            //if (senin.Department != 船員.船員_所属_内航)
            //{
            //    MessageBox.Show("所属が内航はありません。","配乗計画内航");
            //    return;
            //}
            ...
            PutAppointment(newy, cp.RowIndex, dt, 3, true);
        }
        #endregion
        ...
        private void SearchRevision()
        {
            ...
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
                //HeadTbl = serviceClient.BLC_配乗計画_SearchRevisions(NBaseCommon.Common.LoginUser, date1, date2, SiCardPlanHead.VESSEL_KIND_内航);
        ...
        private void SearchSenin()
        {
            ...
            ProgressDialog progressDialog = new ProgressDialog(delegate
            {
                using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
                {
                    ...
                    senins = serviceClient.MsSenin_GetRecords(NBaseCommon.Common.LoginUser);
            ...
            ////職名、氏名コードで並べ替え
            //foreach (object item in comboBox_職種.Items)
            //{
            //    if (!(item is Shokumei)) continue;

            //    Shokumei shoku = item as Shokumei;

            //    List<MsSenin> wklist = senins.Where(obj => obj.MsSiShokumeiID == shoku.MsSiShokumeiID).OrderBy(obj => obj.ShimeiCode).ToList();
            //    foreach (MsSenin s in wklist)
            //    {
            //        SeninList.Add(s);
            //    }
            //}
            SeninList.AddRange(senins);
        }
        #endregion
        ...
        private void SearchPlan()
        {
            ...
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
                ...
                {
                    //PlanList = serviceClient.BLC_配乗計画_SearchPlan(NBaseCommon.Common.LoginUser, start, end, SiCardPlanHead.VESSEL_KIND_内航);
                    PlanList = serviceClient.BLC_配乗計画_SearchPlan(NBaseCommon.Common.LoginUser, start, end, PlanType);
                }
                else
                {
                    //そのヘッダだけ取得
                    //PlanList = serviceClient.BLC_配乗計画_SearchPlanByHeder(NBaseCommon.Common.LoginUser, head, SiCardPlanHead.VESSEL_KIND_内航);
        ...
        private void ActListに入れる(List<SiCard> catdlist)
        {
            foreach (SiCard card in catdlist)
            {
                ...
                {
                    Shokumei shoku = SeninTableCache.instance().GetShokumei(NBaseCommon.Common.LoginUser, Shokumei.内航, ap.MsSiShokumeiID, ap.MsSiShokumeiShousaiID);
        ...
        private void 詳細画面を開く(bool is実績, Appointment ap)
        {
            ...
            int span = this.gcCalendarGrid1[dateFrom][rowindex, 0].ColumnSpan;
            ...
            //期間
            int oldSpan = gcCalendarGrid1.Content[dateFrom][rowindex, 0].ColumnSpan;

            //詳細画面表示
            //配乗計画内航詳細Form frm = new 配乗計画内航詳細Form(newP, Combobox種別List, enable, SeninViewList);
            //配乗計画内航詳細Form frm = new 配乗計画内航詳細Form(PlanType, newP, Combobox種別List, enable, SeninViewList);
            配乗計画詳細Form frm = null;
        ...
        private SiCardPlan InsertUpdate(Appointment ap)
        {
            ...
            else
            {
                ...
                targetP.LaborOnBoarding = 0;
                targetP.LaborOnDisembarking = 0;
            }
            ...
            {
                //登録更新したデータが戻る
                //plan = serviceClient.BLC_配乗計画_InsertOrUpdate(NBaseCommon.Common.LoginUser, targetP, dt, SiCardPlanHead.VESSEL_KIND_内航);
        ...
        private void GetSeninViewList()
        {
            ...
            //            (SeninList.Any(o => o.MsSeninID == seninId && o.Department == 船員.船員_所属_内航) ||
            ...
            //        {
            //            seninIDs1.AddRange(SeninList.Where(o => o.MsSeninID == seninId).Select(o => o.MsSeninID));
            //        }
            //    }
            //}
        ...
        #region public void PutAppointment( Appointment ap, int rowindex, DateTime startdate, int colspan, bool 仮)
        public void PutAppointment(Appointment ap, int rowindex, DateTime startdate, int colspan, bool 仮)
        {
            ...
            //セルタイプクローン、必須。おやくそく
            gcCalendarGrid1.Content[startdate][rowindex, 0].CellType = cactype1.Clone();//日付列は1カラムのため常にcolindex = 0

            //太字
            Font baseFont = gcCalendarGrid1.Font;
            Font fnt = new Font(baseFont.FontFamily, baseFont.Size, baseFont.Style | FontStyle.Bold);
            gcCalendarGrid1.Content[startdate][rowindex, 0].CellStyle.Font = fnt;
            ...
            if (colspan > 0)
            {
                this.gcCalendarGrid1[startdate][rowindex, 0].ColumnSpan = colspan;
            }

            //計画とAppointmentの紐づけ
            this.gcCalendarGrid1.Content[dt][rowindex, 0].Value = ap;

            gcCalendarGrid1.Content[startdate][rowindex, 0].CellStyle.ForeColor = Color.Black;
            gcCalendarGrid1.Content[startdate][rowindex, 0].ToolTipText = ap.SeninName;


            if (仮)//Appointment仮置き 登録されていない白ぬきの
            {
                (this.gcCalendarGrid1[startdate][rowindex, 0].CellType as CalendarAppointmentCellType).Renderer = renderWhite;
            }
            else
            {
                //if (ap.ActFlg == 0)
                ...
                (this.gcCalendarGrid1[startdate][rowindex, 0].CellType as CalendarAppointmentCellType).Renderer = rend;
                if (rend.LineColor != rend.FillColor)
                {
                    //薄い色バーの時は文字も薄くする
                    gcCalendarGrid1.Content[startdate][rowindex, 0].CellStyle.ForeColor = Color.Gray;
                }
                //}
                //else
                //{
