...
namespace Senin
{
    public partial class 配乗計画内航出力Form : Form
    {
        private int PlanType;
        private int PeriodYear = 0;

        public 配乗計画内航出力Form(int planType, int period)
        {
            InitializeComponent();
        ...
        private void 配乗計画内航出力Form_Load(object sender, EventArgs e)
        {
            InitComboBox年();
            InitComboBox月();
        ...
        private void button出力_Click(object sender, EventArgs e)
        {
            ...
            if (radioButton計画.Checked)
            {
                ...
                {
                    //List<SiCardPlanHead> headlist = serviceClient.SiCardPlanHead_GetRecordsByYearMonth(NBaseCommon.Common.LoginUser, dt, SiCardPlanHead.VESSEL_KIND_内航);
                ...
                filename = "乗下船計画表" + "_" + dt.ToString("yyyyMM") + "_" + revno.ToString("D") + ".xlsx";
            }
            else
            {
                filename = "乗下船実績表" + "_" + dt.ToString("yyyyMM") +".xlsx";
            }
