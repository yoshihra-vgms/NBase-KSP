...
namespace Dousei
{
    public partial class 動静報告Form : Form
    {
        ...
        public List<MsVessel> VesselList { set { vesselList = value; } }
        ...
        private void Init()
        {
            ...
            if (DouseiHoukoku != null)
            {
                ...
                foreach(MsVessel vessel in comboBox_船.Items)
                {
                    if (vessel.MsVesselID == DouseiHoukoku.VesselID)
                    {
                ...
                if (DouseiHoukoku.LeaveDate != DateTime.MinValue)
                {
                    ...
                    textBox_出港時間.Text = DouseiHoukoku.LeaveDate.Hour.ToString("00") + DouseiHoukoku.LeaveDate.Minute.ToString("00");
        ...
        private bool ValidateFields()
        {
            ...
            if (textBox_出港時間.Text != null && textBox_出港時間.Text.Length > 0 && !TimeFormatCheck(textBox_出港時間.Text))
            {
                textBox_出港時間.BackColor = Color.Pink;
                MessageBox.Show("出港時間を0000〜2359の範囲で入力してください", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                textBox_出港時間.BackColor = Color.White;
                return false;
            }
            ...
            if (!(comboBox_船.SelectedItem is MsVessel))
            {
                comboBox_船.BackColor = Color.Pink;
                MessageBox.Show("船名を選択して下さい", "入力エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                comboBox_船.BackColor = Color.White;
                return false;
            }
        ...
        private void FillInstance()
        {
            ...
            DouseiHoukoku.VesselID = (comboBox_船.SelectedItem as MsVessel).MsVesselID;

            DouseiHoukoku.LeavePortID = GetPortId(comboBox_出港地);
            DouseiHoukoku.LeaveDate = ConvertDate(nullableDateTimePicker_出港日, textBox_出港時間);
        ...
        private bool InsertOrUpdate()
        {
            ...
            {
                DjDouseiHoukoku retHoukoku = serviceClient.DjDouseiHoukoku_InsertOrUpdate(NBaseCommon.Common.LoginUser, DouseiHoukoku);
                if (retHoukoku != null)
                {
