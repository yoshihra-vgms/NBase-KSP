...
namespace Hachu.Reports
{
    public partial class CSV出力Form : Form
    {
        ...
        /// インスタンスを格納するフィールド変数
        /// </summary>
        private static CSV出力Form instance = null;
        ...
        /// </summary>
        /// <returns></returns>
        public static CSV出力Form GetInstance()
        {
            if (instance == null)
            {
                instance = new CSV出力Form();
            }
            return instance;
        }

        private void CSV出力Form_FormClosing(object sender, FormClosingEventArgs e)
        {
            instance = null;
        }
        ...
        private void CSV出力Form_Load(object sender, EventArgs e)
        {
            this.Text = WingCommon.Common.WindowTitle("番号不明", "CSV出力");
            検索条件初期化();
        }
        ...
        private void 検索条件初期化()
        {
            ...
            {
                vessels = serviceClient.MsVessel_GetRecordsByHachuEnabled(WingCommon.Common.LoginUser);
                customers = serviceClient.MsCustomer_GetRecords(WingCommon.Common.LoginUser);
                thiIraiSbts = serviceClient.MsThiIraiSbt_GetRecords(WingCommon.Common.LoginUser);
                thiIraiShousais = serviceClient.MsThiIraiShousai_GetRecords(WingCommon.Common.LoginUser);
            }
        ...
        private void comboBox種別_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (selected.MsThiIraiSbtID == WingCommon.Common.MsThiIraiSbt_修繕ID)
            {
                MsThiIraiShousai dmyThiIraiShousai = new MsThiIraiShousai();
        ...
        private void button出力_Click(object sender, EventArgs e)
        {
            ...
            {
                検索条件.type = 発注RowData.出力タイプEnum.ヘッダー;
            }
            else
            {
                検索条件.type = 発注RowData.出力タイプEnum.詳細品目;
            }
            #endregion
        ...
        #region public void Output(発注RowData.検索条件 検索条件)
        public void Output(検索条件 検索条件)
        {
            SaveFileDialog fd = new SaveFileDialog();
            fd.Filter = BaseFileName + " (*.xls)|*.xls";
            fd.FileName = BaseFileName + ".xls";
            ...
            if (fd.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    byte[] excelData = null;
                    Hachu.Utils.ProgressDialog progressDialog = new Hachu.Utils.ProgressDialog(delegate
                    {
        ...
        /// <param name="検索条件"></param>
        /// <returns></returns>
        private byte[] MakeFile(検索条件 検索条件)
        {
            byte[] excelData = null;
            using (WingServiceReferences.WingServer.ServiceClient serviceClient = new WingServiceReferences.WingServer.ServiceClient())
            {
                if (検索条件.type == 発注RowData.出力タイプEnum.ヘッダー)
                {
                    excelData = serviceClient.BLC_発注RowDataヘッダー_取得(WingCommon.Common.LoginUser, 検索条件);
                }
                else
                {
                    excelData = serviceClient.BLC_発注RowData詳細品目_取得(WingCommon.Common.LoginUser, 検索条件);
                }
            }
            return excelData;
        }
