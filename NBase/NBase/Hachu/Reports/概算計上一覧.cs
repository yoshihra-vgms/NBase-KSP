...
namespace Hachu.Reports
{
    public class 概算計上一覧
    {
        string BaseFileName = "概算計上一覧";

        public void Output(int year, int month, List<概算計上一覧Row> 概算計上一覧Rows)
        {
        ...
        private byte[] MakeFile(int year, int month, List<概算計上一覧Row> 概算計上一覧Rows)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
                byte[] excelData = serviceClient.BLC_Excel概算計上一覧_取得(NBaseCommon.Common.LoginUser, year, month, 概算計上一覧Rows);
