...
{
    public class Data潤滑油集計 : BaseData
    {
        string BaseFileName = "潤滑油集計表";

        public Data潤滑油集計(string fn, string cn)
        {
            formNumber = fn;
            className = cn;
        }
        ...
        private byte[] MakeFile(int FromYear, List<bool> Targets)
        {
            ...
            {
                byte[] excelData = serviceClient.BLC_貯蔵品集計表_取得(NBaseCommon.Common.LoginUser, FromYear, Targets, NBaseData.BLC.貯蔵品集計表.Enum対象.潤滑油);
