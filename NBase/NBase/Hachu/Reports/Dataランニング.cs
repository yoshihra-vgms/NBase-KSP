...
{
    public class Dataランニング : BaseData
    {
        //string BaseFileName = "ランニング管理表"; 名称変更、ランニング管理表→部品購入管理表　2021/08/05  m.yoshihara
        string BaseFileName = "部品購入管理表";
        string ErrMessage = "";
        bool outputBusy = false;
 
        public Dataランニング(string fn, string cn)
        {
            formNumber = fn;
            className = cn;
        }
        public override void Output(int FromYear, int FromMonth, int ToYear, int ToMonth)
        {
            ...
            if (fd.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    //名称変更にともない修正　2021/08/05  m.yoshihara
                    //-----------------------------------------------------------------------------------
                    ...
                    if (ErrMessage != "")
                    {
                        ...
                        {
                            // 失敗 
                            //message = "ランニング管理表の出力に失敗しました。\n (Err:" + ErrMessage + ")"; 名称変更にともない　2021/08/05  m.yoshihara
                            message = "部品購入管理表の出力に失敗しました。\n (Err:" + ErrMessage + ")";
                    ...
                    if (excelData == null)
                    {
                        //MessageBox.Show("ランニング管理表の出力に失敗しました。\n (テンプレートファイルがありません。管理者に確認してください。)", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);2021/08/05  m.yoshihara
                        MessageBox.Show("部品購入管理表の出力に失敗しました。\n (テンプレートファイルがありません。管理者に確認してください。)", "エラー", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return;
                    }
                ...
                {
                    //message = "ランニング管理表の出力に失敗しました。\n (Err:" + ex.InnerException.Message + ")";2021/08/05  m.yoshihara
        ...
        private byte[] MakeFile(int FromYear, int FromMonth, int ToYear, int ToMonth)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
                try
                {
                    byte[] excelData = serviceClient.BLC_Excelランニング管理表_取得(NBaseCommon.Common.LoginUser, FromYear, FromMonth, ToYear, ToMonth);
