...
{
    public class Data貯蔵品年間管理表 : BaseData
    {
        string BaseFileName = "貯蔵品年間管理表";
        NBaseData.BLC.貯蔵品リスト.対象Enum kind;

        public Data貯蔵品年間管理表(string fn, string cn, NBaseData.BLC.貯蔵品リスト.対象Enum kind)
        {
            this.formNumber = fn;
            this.className = cn;
            this.kind = kind;
        }
        public override void Output(NBaseData.DAC.MsVessel MsVessel, int FromYear)
        {
        ...
        private byte[] MakeFile(NBaseData.DAC.MsVessel MsVessel, int FromYear)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
                byte[] excelData = serviceClient.BLC_Excel貯蔵品年間管理表_出力(NBaseCommon.Common.LoginUser, MsVessel, kind, FromYear);
