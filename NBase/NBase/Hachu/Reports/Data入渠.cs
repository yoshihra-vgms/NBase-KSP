...
{
    public class Data入渠 : BaseData
    {
        string BaseFileName = "入渠費用管理表";
        string ErrMessage = "";
        bool outputBusy = false;

        public Data入渠(string fn, string cn)
        {
            formNumber = fn;
            className = cn;
        }
        ...
        private byte[] MakeFile(int FromYear, int FromMonth, int ToYear, int ToMonth)
        {
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
                try
                {
                    byte[] excelData = serviceClient.BLC_Excel入渠費用管理表_取得(NBaseCommon.Common.LoginUser, FromYear, FromMonth, ToYear, ToMonth);
