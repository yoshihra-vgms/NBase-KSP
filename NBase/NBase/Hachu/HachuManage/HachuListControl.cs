...
{
    public partial class HachuListControl : UserControl
    {
        ...
        public void Init(int width)
        {
            ...
            treeListView発注一覧.ExpandBoxStyle.ImageSize = s;

            // ヘッダ、フッタは見せない
            treeListView発注一覧.Header = false;
            treeListView発注一覧.Footer = false;

            // カラム１（情報を表示）
            TreeListViewColumn column1 = new TreeListViewColumn();
            column1.HeaderText = "情報";
            column1.Width = width;
            treeListView発注一覧.Columns.Add(column1);
            treeListView発注一覧.Columns[0].ContentType = ColumnContentType.Custom;

            // 色がつくのは文字の開始位置から
            treeListView発注一覧.ItemHighlightType = ListItemHighlightType.Partial;
            ...
            List<OdShr> OdShrs)
        {
            ...
            foreach (ListInfo手配依頼 thiInfo in 手配List)
            {
                ...
                thiNode.Tag = thiInfo;
                treeListView発注一覧.Nodes.Add(thiNode);
            }
        ...
        public void RemoveSelectedNode()
        {
            try
            {
                ...
                TreeListViewNode node = treeListView発注一覧.SelectedNode;
                treeListView発注一覧.Nodes.Remove(node);
            }
            catch
            {
            }
        ...
        {
            TreeListViewNode selectedNode = treeListView発注一覧.SelectedNode;
        ...
        {
            int c = treeListView発注一覧.Nodes.Count;
            if (c == 0) return;

            TreeListViewNode node = treeListView発注一覧.Nodes[c-1];

            treeListView発注一覧.EnsureVisible(node);
        }
