...
{
    public partial class 見積回答Form : BaseUserControl //2021/07/12 BaseForm
    {
        private OdMm 対象見積依頼;
        ...
        private class saveShousaiItem
        {
            ...
            public saveShousaiItem(string id, int count, decimal tanka)
            {
                this.id = id;
                this.count = count;
                this.tanka = tanka;
        ...
        //private float panelMaxHeight = 434 + 6;
        private float panelMinHeight = 0;
        private float panelMaxHeight = 305;
        ...
        #region public 見積回答Form(int windowStyle, ListInfo見積回答 info)
        public 見積回答Form(int windowStyle, ListInfo見積回答 info)
        {
            InitializeComponent();
        ...
        #region public 見積回答Form(int windowStyle, OdMk info, OdMm parent)
        public 見積回答Form(int windowStyle, OdMk info, OdMm parent)
        {
            InitializeComponent();
        ...
        private void button注文書出力_Click(object sender, EventArgs e)
        {
            ...
            //        対象見積依頼 = serviceClient.OdMm_GetRecord(NBaseCommon.Common.LoginUser, 対象見積回答.OdMmID);
            //        対象見積回答 = serviceClient.OdMk_GetRecord(NBaseCommon.Common.LoginUser, 対象見積回答.OdMkID);
            //    }
            //    ListInfo見積回答 info = new ListInfo見積回答();
        ...
        private void textBox見積値引き_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            try
            {
                preNebiki = NBaseCommon.Common.金額表示を数値へ変換(textBox見積値引き.Text);
            }
            catch
            {
            }
        ...
        private void textBox消費税_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            try
            {
                preTax = NBaseCommon.Common.金額表示を数値へ変換(textBox消費税.Text);
            }
            catch
            {
            }
        ...
        private void button品目編集_Click(object sender, EventArgs e)
        {
            ...
            {
                燃料潤滑油編集Form form = new 燃料潤滑油編集Form(見積回答品目s);
                if (form.ShowDialog() == DialogResult.Cancel)
                {
                    return;
                }
        ...
        private void Formに情報をセットする()
        {
            ...
            using (ServiceReferences.NBaseService.ServiceClient serviceClient = ServiceReferences.WcfServiceWrapper.GetInstance().GetServiceClient())
            {
                ...
                thi = serviceClient.OdThi_GetRecordByOdMkID(NBaseCommon.Common.LoginUser, 対象見積回答.OdMkID);

                mm = serviceClient.OdMm_GetRecord(NBaseCommon.Common.LoginUser, 対象見積回答.OdMmID);
                if (mm != null)
                {
            ...
            //=========================================
            #region Windowタイトル
            this.Text = NBaseCommon.Common.WindowTitle("JM040401", "見積回答", ServiceReferences.WcfServiceWrapper.ConnectedServerID);
            #endregion
            #region 入力エリア
            ...
            textBox消費税.Text = NBaseCommon.Common.金額出力(対象見積回答.Tax);
            ...
            if (WindowStyle == (int)WINDOW_STYLE.通常)
            {
                ...
                else if (対象見積回答.Status == 対象見積回答.OdStatusValue.Values[(int)OdMk.STATUS.回答済み].Value)
                {
                    ...
                    if (NBaseCommon.Common.発注承認しきい値 > 0 &&
                        対象見積回答.Amount > NBaseCommon.Common.発注承認しきい値)
                    {
                        button発注.Visible = false;
        ...
        private void InitItemTreeListView()
        {
            ...
            品目TreeList = new ItemTreeListView見積回答(対象見積回答.MsThiIraiSbtID, treeListView);
            if (対象見積回答.MsThiIraiSbtID == NBaseCommon.Common.MsThiIraiSbt_燃料潤滑油ID)
            {
                品目TreeList.Enum表示方式 = ItemTreeListView見積回答.表示方式enum.Zero以外を表示;
            }
            // <==
        ...
        // 2016.10 Add
        private bool Validate変更()
        {
            List<Item見積回答品目> Orgs = Item見積回答品目.GetRecords(対象見積回答.OdMkID);
            List<saveShousaiItem> saveShousaiItemList = new List<saveShousaiItem>();
            foreach (Item見積回答品目 見積回答品目 in Orgs)
            {
                foreach (OdMkShousaiItem 詳細品目 in 見積回答品目.詳細品目s)
                {
                    saveShousaiItemList.Add(new saveShousaiItem(詳細品目.OdMkShousaiItemID, 詳細品目.Count, 詳細品目.Tanka));
                }
            } 
            ...
            foreach (Item見積回答品目 見積回答品目 in 見積回答品目s)
            {
                foreach (OdMkShousaiItem 詳細品目 in 見積回答品目.詳細品目s)
                {
                    if (saveShousaiItemList.Any(obj => obj.id == 詳細品目.OdMkShousaiItemID))
                    {
                        var item = saveShousaiItemList.Where(obj => obj.id == 詳細品目.OdMkShousaiItemID).First();
                        if (item.count == 詳細品目.Count && item.tanka == 詳細品目.Tanka)
                        {
                            item.check = true;
                        }
                    }
            ...
            if (saveShousaiItemList.Any(obj => obj.check == false))
            {
                MessageBox.Show("回答数または単価が変更されています。" + System.Environment.NewLine + "更新ボタンをクリックして下さい。", "確認", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
            return true;
        }
