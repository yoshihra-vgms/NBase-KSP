...
//using static NBaseCommon.Hachu.Excel.発注帳票Common;

namespace NBaseCommon.Hachu.Excel
{
    public class KK船用品請求書出力 : 発注帳票Common
    {
        ...
        public KK船用品請求書出力(string templateFilePath, string outputFilePath, string outputPdfFilePath)
        {
        ...
        private void _CreateFile(MsUser loginUser, ExcelCreator.Xlsx.XlsxCreator xls, 発注帳票Common.kubun発注帳票 kubun)
        {
            ...
            foreach (string itemSbtId in itemSbtIds)
            {
                ...
                foreach (string key in shousaiItemList.Keys)
                {
                    ...
                    xls.CopySheet(tempPageNo, pages + 1, "");
                    xls.SheetNo = pages + 1;
                    ...
                    if (keys.Count() + 1 + shousaiItemList[key].Count() > rowsPerPageMax)
                    {
                        int rows = (keys.Count() + 1 + shousaiItemList[key].Count()) - rowsPerPageMax;
                    ...
                    do
                    {
                        ...
                        for (int i = 0; i < maxRows; i++)
                        {
                            ...
                            // 単位
                            xls.Cell("**単位" + (rowNo).ToString()).Value = shousaiItem.TaniName;
                            ...
                            // 依頼数
                            xls.Cell("**数量" + (rowNo).ToString()).Value = shousaiItem.Count.ToString();

                            // 査定数
                            if (kubun == 発注帳票Common.kubun発注帳票.査定表)
                            {
                                xls.Cell("**査定数" + (rowNo).ToString()).Value = shousaiItem.Satei.ToString();
                            }
