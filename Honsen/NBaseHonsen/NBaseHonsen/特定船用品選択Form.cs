...
namespace NBaseHonsen
{
    public partial class 特定船用品選択Form : Form
    {
        ...
        {
            this.Text = NBaseCommon.Common.WindowTitle("番号不明", "特定船用品選択", WcfServiceWrapper.ConnectedServerID);

            textBox_カテゴリ.Text = SelectedVesselItemCategory.CategoryName;

            MsVesselItemVesselList = MsVesselItemVessel.GetRecordsByMsVesselIDVesselItemName(SyncClient.同期Client.LOGIN_USER, MsVesselID, SelectedVesselItemCategory.CategoryNumber, "");
            
            // 前月の貯蔵レコードを取得する
            //string ym = DateTime.Today.AddMonths(-1).ToString("yyyyMM");
            //OdChozoShousaiList = OdChozoShousai.GetRecordsByVesselID_Date_Shubetsu(SyncClient.同期Client.LOGIN_USER, MsVesselID, ym, (int)OdChozo.ShubetsuEnum.特定品);
            OdChozoShousaiList = 船用品.BLC_Get特定船用品在庫(SyncClient.同期Client.LOGIN_USER, MsVesselID);
