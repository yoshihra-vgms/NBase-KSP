    ...
    public partial class 給与手当登録Form : Form
    {
        ...
        private void 給与手当登録Form_Load(object sender, EventArgs e)
        {
            ...
            if (siKyuyoTeate != null)
            {
                ...
                comboBox給与手当.SelectedItem = SeninTableCache.instance().GetMsSiKyuyoTeate(同期Client.LOGIN_USER, siKyuyoTeate.MsSiKyuyoTeateID);
                ...
                textBox金額.Text = siKyuyoTeate.HonsenKingaku != int.MinValue ? siKyuyoTeate.HonsenKingaku.ToString() : "";
        ...
        private void InitComboBox職名()
        {
            ...
            foreach (MsSiShokumei s in SeninTableCache.instance().GetMsSiShokumeiList(同期Client.LOGIN_USER))
            {
        ...
        private void InitComboBox給与手当()
        {
            ...
            foreach (MsSiKyuyoTeate m in SeninTableCache.instance().GetMsSiKyuyoTeateList(同期Client.LOGIN_USER))
            {
        ...
        private void comboBox給与手当_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            List<MsSiKyuyoTeateSet> kyuyoTeateSetList = SeninTableCache.instance().GetMsSiKyuyoTeateSetList(同期Client.LOGIN_USER);
        ...
        private bool InsertOrUpdate()
        {
            ...
            using (DBConnect dbConnect = new DBConnect())
            {
                ...
                try
                {
                    if (siKyuyoTeate.IsNew())
                    {
                        siKyuyoTeate.SiKyuyoTeateID = System.Guid.NewGuid().ToString();
                    }
                    siKyuyoTeate.RenewUserID = 同期Client.LOGIN_USER.MsUserID;
                    siKyuyoTeate.RenewDate = DateTime.Now;

                    SyncTableSaver.InsertOrUpdate(siKyuyoTeate, 同期Client.LOGIN_USER, StatusUtils.通信状況.未同期, dbConnect);
                    dbConnect.Commit();

                    SiKyuyoTeate retKuyoTeate = SiKyuyoTeate.GetRecord(同期Client.LOGIN_USER, siKyuyoTeate.SiKyuyoTeateID);
        ...
        private void FillInstance()
        {
            ...
            siKyuyoTeate.HonsenKingaku = int.Parse(textBox金額.Text);
            if (siKyuyoTeate.IsNew())
                siKyuyoTeate.Kingaku = siKyuyoTeate.HonsenKingaku;

            siKyuyoTeate.VesselID = 同期Client.LOGIN_VESSEL.MsVesselID;
        }
        #endregion
