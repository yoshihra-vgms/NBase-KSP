    ...
    public partial class tek手当詳細Form : Form
    {
        ...
        private void tek手当詳細Form_Load(object sender, EventArgs e)
        {
            ...
            if (siAllowance != null)
            {
                ...
                siAllowanceDetails = SiAllowanceDetail.GetRecords(NBaseCommon.Common.LoginUser, siAllowance.SiAllowanceID);
        ...
        private void InitComboBox手当名()
        {
            foreach (MsSiAllowance a in SeninTableCache.instance().GetMsSiAllowanceList(NBaseCommon.Common.LoginUser))
            {
                ...
                if (vesselIds.Contains(同期Client.LOGIN_VESSEL.MsVesselID))
        ...
        {
            comboBox船名.Items.Add(SeninTableCache.instance().GetMsVessel(NBaseCommon.Common.LoginUser, 同期Client.LOGIN_VESSEL.MsVesselID));
        ...
        private void Bind()
        {
            ...
            if (allowance == null || vessel == null || ym == null)
            {
                return;
            }
            ...
            {
                tmpcards = SiCard.GetRecordsByFilter(NBaseCommon.Common.LoginUser, cardFilter);
        ...
        private bool InsertOrUpdate()
        {
            ...
            siAllowance.RenewDate = DateTime.Now;
            siAllowance.RenewUserID = 同期Client.LOGIN_USER.MsUserID;
            ...
            using (DBConnect dbConnect = new DBConnect())
            {
                ...
                try
                {
                    ...
                    {
                        siAllowance.SiAllowanceID = System.Guid.NewGuid().ToString();
                    }

                    SyncTableSaver.InsertOrUpdate(siAllowance, 同期Client.LOGIN_USER, StatusUtils.通信状況.未同期, dbConnect);

                    if (ret)
                    {
                        foreach (SiAllowanceDetail detail in siAllowanceDetails)
                        {
                            detail.SiAllowanceID = siAllowance.SiAllowanceID;
                            detail.RenewDate = siAllowance.RenewDate;
                            detail.RenewUserID = siAllowance.RenewUserID;
                            ...
                            SyncTableSaver.InsertOrUpdate(detail, 同期Client.LOGIN_USER, StatusUtils.通信状況.未同期, dbConnect);
                        }
        ...
        private void Output()
        {
            ...
            saveFileDialog1.FileName = baseFileName + ".xlsx";

            if (saveFileDialog1.ShowDialog() == DialogResult.OK)
            {
                string exeDir = System.IO.Directory.GetCurrentDirectory();
                string templateFilePath = exeDir + "\\Template\\Template_" + baseFileName + ".xlsx";
                string outputFilePath = saveFileDialog1.FileName;

                new tek手当支給依頼書出力(templateFilePath, outputFilePath).CreateFile(NBaseCommon.Common.LoginUser, SeninTableCache.instance(), siAllowance, siAllowanceDetails);

                string smes = "「" + outputFilePath + "」に出力しました。";
                MessageBox.Show(smes, "手当登録", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        }
