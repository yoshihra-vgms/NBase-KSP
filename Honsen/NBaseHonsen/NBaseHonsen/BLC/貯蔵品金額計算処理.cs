...
namespace NBaseHonsen.BLC
{
    ...
    public class 貯蔵品金額計算処理
    {
        ...
        public static void 貯蔵品金額計算総括(List<貯蔵品TreeInfo> datalist, int year, int month)
        {
            ...
            {
                lolist = sc.貯蔵品リスト_GetRecords(NBaseCommon.Common.LoginUser, year, month, 貯蔵品リスト.Enum対象.潤滑油);
                itemlist = sc.貯蔵品リスト_GetRecords(NBaseCommon.Common.LoginUser, year, month, 貯蔵品リスト.Enum対象.船用品);
            }*/
        ...
        public static void CalcuUkeireDate(int year, int month)
        {
            ...
            veslist = MsVessel.GetRecordsByHachuEnabled(NBaseCommon.Common.LoginUser);
                    ...
                    foreach (貯蔵品TreeInfo trdata in datalist)
                    {
                        ...
                        foreach (貯蔵品TreeInfo pre in prelist)
                        {
                                ...
                                pre.OdChozoShousaiData.MsVesselItemID == trdata.OdChozoShousaiData.MsVesselItemID)
                            {
                                //一致したら受け入れ年月を設定して終わり
                                int dummy = 0;
                                DateTime da = 貯蔵品金額計算処理.ChozoShousai計算(pre, year, month, out dummy, false);
                                trdata.OdChozoShousaiData.UkeireNengetsu = da.ToString("yyyyMM");


                                //設定したデータを挿入する
                                trdata.OdChozoShousaiData.UpdateRecord(NBaseCommon.Common.LoginUser);
        ...
        private static DateTime ChozoShousai計算(貯蔵品TreeInfo treedata, int year, int month, out int amount, bool work)
        {
            ...
            //検索する対象の判定
            /*string thi_state = MsThiIraiSbt.ToId(MsThiIraiSbt.ThiIraiSbtEnum.燃料_潤滑油);
            ...
            for (int i = 0; i < smon + 1; i++)
            {
                ...
                {
                    //貯蔵品詳細データの取得
                    chozolist = OdChozoShousai.GetRecordsByVesselID_Period_MsLoID(NBaseCommon.Common.LoginUser, data.MsVesselID,
                    ...
                    jrylist = OdJryShousaiItem.GetRecordsByMsVesselID_JryStatus_ShousaiID_Date_MsLoID(
                        NBaseCommon.Common.LoginUser, MsThiIraiSbt.ToId(MsThiIraiSbt.ThiIraiSbtEnum.燃料_潤滑油), 1, data.MsVesselID, sd, ed, data.MsLoID);
                ...
                if (data.MsVesselItemID.Length > 0)
                {
                    ...
                    //貯蔵品詳細データの取得
                    chozolist = OdChozoShousai.GetRecordsByVesselID_Period_MsVesselItemID(NBaseCommon.Common.LoginUser, data.MsVesselID,
                    ...
                    jrylist = OdJryShousaiItem.GetRecordsByMsVesselID_JryStatus_ShousaiID_Date_MsVesselItemID(
                        NBaseCommon.Common.LoginUser, MsThiIraiSbt.ToId(MsThiIraiSbt.ThiIraiSbtEnum.燃料_潤滑油), 1, data.MsVesselID, sd, ed, data.MsVesselItemID);
                }
                #endregion
