...
namespace DcCommon.DB.DAC.Search
{
    ...
    /// 会社予定の検索
    /// </summary>
    public class ScheduleCompanySearchData : BaseSearchData
    {
        #region メンバ変数
        ...
        public override string CreateSQLWhere(out List<SqlParamData> plist)
        {
...

            {
                ...
                if (this.estimate_date_start != null && this.estimate_date_end != null)
                {
                    ...
                    datesql += @"
(dc_schedule_company.estimate_date BETWEEN :estimate_date_start AND :estimate_date_end)
";
...
OR";
                    ...
                    datesql += @"
(dc_schedule_company.inspection_date BETWEEN :inspection_date_start AND :inspection_date_end)
";
...
OR";
                    ...
                    datesql += @"
(dc_schedule_company.expiry_date BETWEEN :expiry_date_start AND :expiry_date_end)
";
...
({0})", datesql);
            ...
            if (this.output_expiry_date != null)
            {
                ans += @"
AND
(dc_schedule_company.expiry_date >= :output_expiry_date)
";
